...
$transfer_history = $conn->query("
    ...
    ORDER BY b1.name, b2.name, t.transfer_date DESC
");

// Group data by branch
$grouped_history = [];
while ($row = $transfer_history->fetch_assoc()) {
    $branch_name = $row['to_branch'] ?? $row['from_branch'] ?? 'N/A';
    $grouped_history[$branch_name][] = $row;
}
?>
...
<div class="card">
<div class="card-body"> 
<h5 class="card-title mb-9 fw-semibold">Transfer History (Grouped by Branch) </h5>

        
        <?php foreach ($grouped_history as $branch_name => $records): ?>
            <h4><?= $branch_name ?></h4>
